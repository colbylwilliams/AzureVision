...
public class AzureVisionClient {
    ...
    fileprivate var region: AzureRegion = .westUS
    fileprivate var subscriptionKey: String = Keys.subscriptionKeyDefault
    
    
    // TODO: add region to plist options
    public func configure(_ subscriptionKey: String? = nil, region: AzureRegion = .westUS) {
        
        if !subscriptionKey.isNilOrEmpty {
            self.subscriptionKey = subscriptionKey!
        }
        self.region = region
    }
    
    public func configure(withPlistNamed customPlistName: String? = nil, region: AzureRegion = .westUS) {
        
        if let keys = Keys.tryCreateFromPlists(custom: customPlistName) {
            
            if keys.hasValidSubscriptionKey {
                self.subscriptionKey = keys.subscriptionKey!
            }
        }
        self.region = region
    }
    ...
    fileprivate func _dataRequest(for api: Api, withBody body: Data? = nil, withQuery query: String? = nil, andHeaders headers: [String:String]? = nil) throws -> URLRequest {
        ...
        guard let url = api.url(forRegion: region, withQuery: query) else { throw AzureVisionClientError.urlError(api.urlString(forRegion: region, withQuery: query)) }
